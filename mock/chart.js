...
for(let i = 0; i < data.data.length; i++) {
  for(let j = 0; j < data.columns.length; j++) {
    if(data.columns[j] == '超额贡献') {
      exContribution.push(data.data[i][j]);
    }
    if(data.columns[j] == '行业配置') {
      configData.push(data.data[i][j]);
    }
    if(data.columns[j] == '选股+交叉') {
      stockcrossData.push(data.data[i][j]);
    }
  }
}
